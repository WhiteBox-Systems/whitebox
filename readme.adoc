...
To open the packages folder, navigate to the main menu at the top of the sublime window and press `Preferences > Browse Packages...`. 
Open the parent folder `sublime-text` (or `sublime-text-3`) then open the `Installed Packages` folder. 
Place `whitebox.sublime-package` into the `Installed Packages` folder
