...
// TODO: library installation
TIP: If installing on Linux, run the following commands from the `whitebox` directory: +
`chmod +x whitebox` +
`chmod +x dbg_run` +
This will ensure the executables can run properly.
...
image::gui_data_tree_changes_graph.png[]
...
image::gui_data_tree_options.png[]
...
=== Status bar
image::gui_status_bar.png[]
image::gui_status_bar_error.png[]
